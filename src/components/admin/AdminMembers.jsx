...
const memberCard = [
  {
    ...
    about:
      "qwerty jhava iu siu aiusgu asiug asg iaguhsn iuash iuags nwiujsg biu",
    year: "2022",
    img: PP,
...
function AdminMembers() {
  ...
  const deleteMember = async (email) => {
    const resp = await axios.delete(`${url}api/members/removeMember/${email}`, {
  ...
  const editMembers = async () => {
    ...
    const resp = await axios.patch(
      `${url}api/members/updateMember/${showModal.id}`,
      dataAdd,
      { headers: { Authorization: `${authCtx.token}` } }
    );
    // console.log(resp);
        ...
        <div className="grid grid-cols-3">
          ...
          <div className="py-2 px-4">
            ...
            >
              ...
              <option value="Assistant Coordinator">
        ...
        <div className="w-full py-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-items-center">
          {showMemData.map((value, key) => {
            return (
              <div
                key={key}
                className="bg-[#111111] w-[80%] sm:w-[70%] md:w-[80%] lg:w-[90%] xl:w-[80%] space-y-4 mx-10 my-5 px-8 py-8 flex flex-col items-center rounded-2xl border-t-4 border-b-4 border-yellow-500"
              >
                <div className="">
                  <img
